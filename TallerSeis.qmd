...
# 📌 2. Reemplazar NA por la media de cada columna (en caso de que haya quedado alguno)
...
# 📌 4. Normalizar los datos numéricos
df_scaled <- df_clustering %>% select(starts_with("SC")) %>% scale()

kable(head(df_clustering))
```
...
```{r,echo=FALSE}
# 📌 5. Determinar el número óptimo de clusters usando el índice de Caliński-Harabasz
set.seed(42)
...
```{r, echo=FALSE}
# 📌 6. Gráfico mejorado del índice de Caliński-Harabasz
...
# 📌 7. Aplicar K-means con el número óptimo de clusters
set.seed(42)
...
# 📌 2. Reemplazar NA por la media de cada columna (en caso de que haya quedado alguno)
df <- data %>% mutate(across(where(is.numeric), ~ifelse(is.na(.), mean(., na.rm = TRUE), .)))

# Seleccionar solo PAIS, Unipais y columnas que inician con "SC"
...
Aquí podemos evidenciar que el número óptimo de grupos según el índice de Hartigan es 4, ya que del grupo 3 al 4 hay un salto muy grande (aprox 182) y a partir de ahí se puede decir que se estabiliza la variación de los grupos.
